...
import { createVNode, type Component, render, type VNode } from "vue";
...
export const createToast = (
  ...
  setupVNode(
    id,
    ToastContentType.TITLE_DESCRIPTION,
    initializedOptions,
    initializedContent
  );
    ...
    if (contentType === ToastContentType.VNODE) {
      ...
      toastVNode = createVNode(
        ...
        setupVNodeProps(
          ...
          verticalOffset,
          close,
          content as DisplayContentObject
        )
      );
    } else {
      toastVNode = createVNode(
...
export const initializeContent = (
  ...
  const text =
    typeof content === "string" ? content : (content as ContentObject).title;

  const description =
    typeof content === "string"
      ? undefined
      : (content as ContentObject).description;
